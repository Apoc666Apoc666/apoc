...
final class Controller
{
    ...
    public function upload(): void
    {
        ...
        // rate limit
        RateLimit::hit('upload:' . $uid, 10, 600); // 10 uploads / 10 min
        ...
        // Decode via GD (security: rejects fake images)
        ...
        $thumbRel  = 'thumbs/' . date('Y/m/') . $base . '.jpg'; // thumbs as jpg for consistency
        ...
        // Compute sha on stored bytes (stable)
        $sha = hash_file('sha256', $uploadAbs);
        $existingId = GalleryRepository::existsByOwnerAndSha($uid, $sha);
        if ($existingId) {
            imagedestroy($img);
            // We do NOT delete the file here (safe behavior); you can later add cleanup logic.
    ...
    private function serve(bool $thumb): void
    {
        ...
        // Authorization: owner OR same couple
        $cid = CoupleRepository::activeCoupleIdForUser($uid);
